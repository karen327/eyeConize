//
//  ThirdTeachingViewController.swift
//  MindReader
//
...
class ThirdTeachingViewController: UIViewController {
    ...
    func startTimer() {
        Timer.scheduledTimer(withTimeInterval: 5, repeats: true) { [weak self] timer in
            ...
            if self.imageViewArray.allSatisfy({ $0.tintColor == .green }) {
                ...
                self.performSegue(withIdentifier: "goToFourthTeachingView", sender: self)
                // Stop the timer
                timer.invalidate()
            }
        }
